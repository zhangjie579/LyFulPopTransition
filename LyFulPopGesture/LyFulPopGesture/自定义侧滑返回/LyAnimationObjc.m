...
{
    LyAnimationObjc * ac = [[LyAnimationObjc alloc]init];
...
//    UIViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    ...
    if (self.navigationOperation == UINavigationControllerOperationPush) {
        ...
        //将截图添加到导航栏的View所属的window上,screentImgView为fromView
    ...
    if (self.navigationOperation == UINavigationControllerOperationPop)
    {
        ...
        //这是toView,开始设置它的x在屏幕左边ScreenWidth的位置，然后将它慢慢移动到0
        ...
        lastVcImgView.clipsToBounds = NO;
        lastVcImgView.layer.shadowColor = [UIColor blackColor].CGColor;
        lastVcImgView.layer.shadowOffset = CGSizeMake(0.8, 0);
        lastVcImgView.layer.shadowOpacity = 0.6;
        lastVcImgView.layer.shadowRadius = 1.2;
...
- (UIImage *)screenShot
{
    ...
    if (self.currentTabBarControl)
    {
        self.navigationController.topViewController.tabBarController.view.backgroundColor = [UIColor clearColor];
        [self.navigationController.topViewController.tabBarController.view drawViewHierarchyInRect:rect afterScreenUpdates:NO];
    }
    else
    {
